...
def plotearCategoriasPosicionesImagenes(img, cat_pos, cat_img):
    if cat_pos.hasValue:
        plotear(img, cat_pos.value.position, cat_img.value.arrayOfImages, cat_pos.value.countItems)
    else:
        for st in cat_pos.subTypes:
            plotearCategoriasPosicionesImagenes(img, cat_pos, cat_img[st.name])

def plotear(img, position, arrayOfImages, countItems):
