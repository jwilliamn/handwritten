...
# Hyperparameters
depth = 64 # K = 32 number of filters
...
with tf.name_scope("Summaries"):
	...
	#tf.summary.scalar('Accu',accuracy(valid_pred.eval(), valid_labels))
...
with tf.Session() as sess:
	...
	for i in range(n_epochs): # train the model n_epochs times
		...
		for j in range(n_batches):
			...
			_, loss_batch, preds, summary = sess.run([trainStepOpt, loss, train_pred, summary_op], feed_dict={X:X_batch, Y:Y_batch, keep_prob:0.5})
	...
	pickle_file = os.path.join(model_path, 'modelD1_param_test.pickle')
